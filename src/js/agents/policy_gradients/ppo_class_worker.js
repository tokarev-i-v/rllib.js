...
function start(e){
        ...
        if (policy_nn){
            ...
            model = build_full_connected(obs_dim, [32,32], act_dim, 'tanh', 'tanh');            
        }
        self.ppo_obj = new PPO({env: env, agent: agent, hidden_sizes:[32,32], cr_lr:5e-4, ac_lr:5e-4, gamma:0.99, lam:0.95, steps_per_env:2000, 
            number_envs:1, eps:0.15, actor_iter:10, critic_iter:10, num_epochs:5000, minibatch_size:256, policy_nn: model});
        self.ppo_obj.train();
}
